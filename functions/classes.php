<?php
                    ...
                    <div id="navMenubd-example" class="navbar-menu">
	    				<div class="navbar-start">';
                    		switch ($Login->IsLogged()) {
                    			case true:
    								$PDO = $Load->DataBase();
    								$con = $PDO->query($Tables->SelectFrom('type_use', 'users WHERE id_use LIKE '.$_SESSION['id'].' AND status_use = 1', 0, 1)) or die ($PDO);
    								while($row = $con->fetch(PDO::FETCH_OBJ)){
					    				switch ($row->type_use){
											case 1:
												#diretor
    ...
    class Pages {
								    ...
								    while($row = $con->fetch(PDO::FETCH_OBJ)){
								    	...
								    	switch ($name_page) {
								    		...
								    		case 'users':
								    			...
								    			switch ($row->type_use) {
								    				case 1: $tipo = 'Diretor'; break;
								    				case 2: $tipo = 'Coordenador'; break;
								    				case 3: $tipo = 'Professor'; break;
								    				case 4: $tipo = 'Funcionário'; break;
								    				case 5: $tipo = 'Aluno'; break;
								    			}
								    			$col_4 = $tipo;
								    			$col_5 = '<a href="'.$action_link.'" class="button is-'.$action_color.' is-small">'.$action_button.'</a>';
								    		break;
		...
		function LoadArticlePage($name_page){
			...
			$PDO = $Load->DataBase();
			$Tables = new Tables;
			#echo $Tables->SelectFrom(null, $name_page);
		    $query = $PDO->query($Tables->SelectFrom(null, $name_page)) or die ($PDO);
		    $cont = $Tables->CountViewTable('default', $name_page);
